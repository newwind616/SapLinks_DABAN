...
{
    public class DICN : DataConvert
    {
        public DICN(CompanyObject action) : base(action)
        {
        }
        ...
        public override void GetData()
        {
            ...
            string sql = string.Format("exec DABAN_BPM_DICN.DBO.P_GetSAPInfo @COMPANY ='{0}'", context.company);
            DataTable dt = SQLHelper.ExecuteDataset(context.connStr, System.Data.CommandType.Text, sql).Tables[0];
            LogInfo.Log.Info("《DICN个人经费》获取需处理数量：" + dt.Rows.Count + "条");
            if (dt.Rows.Count == 0)
                return;
