...
namespace SAPLinks.Bussiness.PersonalFunds.DICG
{
    public class DICG : DataConvert
    {
        ...
        public override void GetData()
        {
            ...
            string sql = string.Format("exec DABAN_BPM_DICG.DBO.P_GetSAPInfo @COMPANY ='{0}'", context.company);
            DataTable dt = SQLHelper.ExecuteDataset(context.connStr, System.Data.CommandType.Text, sql).Tables[0];
            LogInfo.Log.Info("《DICG个人经费》获取需处理数量：" + dt.Rows.Count + "条");
            if (dt.Rows.Count == 0)
                return;
