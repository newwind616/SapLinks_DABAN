...
FROM DABAN_BPM_DICS.DBO.ZSK_C_H AA
LEFT JOIN BPMDB.DBO.SAP_COMPANYFUNDS_LINKS_QUEUE CC ON AA.APPLY_NO=CC.APPLY_NO
WHERE CC.COMPANY='DICS' AND CC.ISLINK=0 AND CC.ISLINK=0
ORDER BY CC.ID ASC,AA.APPLY_NO ASC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (pay_obj_type == "0")
                {
                    ...
                    //    headerText = string.Format("{0}{1}暂借{2}{3}费用", apply_DisplayName, apply_Date.ToString("yyyy年MM月").Substring(2), dk_pay_obj_name, sap_cust_code);
                    //}
                    //else
