...
namespace SAPLinks.Bussiness.CompanyFunds.DSC_B
{
    public class FKTZS_ZSK : DataConvert, IDataAction
    {
        public void GetData(DSC_B_Action dics_Action)
        {
...
AA.SAP_CUST_CODE,
DBO.GETSAPFININFOCODE(AA.SAP_CUST_CODE,'DSC_B',2) CSR_ABB,
--DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWBS,'',0) SAP_VEN_NEWBS,
--DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWUM,'',0) SAP_VEN_NEWUM,
--AA.SAP_VEN_CODE,
DBO.GETSAPFININFOCODE(CC.PAY_OBJ_ID,'DSC_B',1) SUPP_ABB,
CC.DK_PAY_OBJ_NAME,CC.PAY_MODE,
dbo.getFINDisplayName(AA.TASKID) FINDISPLAYNAME,
AA.ABSTRACT
FROM DABAN_BPM_DSC_B.DBO.FKTZS_C_H AA
LEFT JOIN DABAN_BPM_DSC_B.DBO.FKTZS_KM_C_D BB ON AA.TASKID=BB.TASKID
LEFT JOIN DABAN_BPM_DSC_B.DBO.ZSK_C_H CC ON AA.ZSK_NO=CC.APPLY_NO
LEFT JOIN BPMDB.DBO.SAP_COMPANYFUNDS_LINKS_QUEUE DD ON AA.APPLY_NO=DD.APPLY_NO
LEFT JOIN (SELECT TASKID,SUM(APPLY_AMOUNT) APPLY_AMOUNT FROM DABAN_BPM_DSC_B.DBO.FKTZS_ZSK_D GROUP BY TASKID) EE ON AA.TASKID=EE.TASKID
...
ORDER BY DD.ID ASC,AA.APPLY_NO ASC,BB.ID ASC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                //if (pay_obj_type == "0")
                //{
                //申请人（4位）+支付/暂借/销账（2位）+供应商名称（10位）+冒号（1位）+摘要（4位）+空格（1位）+初审人名字（3位）
                headerText = headerText = string.Format("{0}销账{1}：{2} {3}", apply_DisplayName.SubstringExt(4), pay_obj_name.SubstringExt(10), _abstract.SubstringExt(4), findisplayname.SubstringExt(3));
...
SELECT INV_NO FROM DABAN_BPM_DSC_B.DBO.FKTZS_ZP_C_D AA
...
DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWUM,'',0) SAP_VEN_NEWUM,
AA.SAP_VEN_CODE,
DBO.GETSAPFININFOCODE(AA.SAP_VEN_CODE,'DSC_B',1) SUPP_ABB 
FROM DABAN_BPM_DSC_B.DBO.FKTZS_C_H AA
