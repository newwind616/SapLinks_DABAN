...
AA.ABSTRACT
FROM DABAN_BPM_DSC_B.DBO.ZSK_C_H AA
LEFT JOIN BPMDB.DBO.SAP_COMPANYFUNDS_LINKS_QUEUE CC ON AA.APPLY_NO=CC.APPLY_NO
WHERE CC.COMPANY='DSC_B' AND CC.ISLINK=0 AND CC.ISLINK=0
ORDER BY CC.ID ASC,AA.APPLY_NO ASC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                headerText = headerText = string.Format("{0}暂借{1}：{2} {3}", apply_DisplayName.SubstringExt(4), pay_obj_name.SubstringExt(10), _abstract.SubstringExt(4), findisplayname.SubstringExt(3));
                bodyText = string.Format("{0}暂借{1}：{2}", apply_DisplayName, pay_obj_name, _abstract);
