...
AA.ABSTRACT
FROM DABAN_BPM_DSCG.DBO.ZSK_C_H AA
LEFT JOIN BPMDB.DBO.SAP_COMPANYFUNDS_LINKS_QUEUE CC ON AA.APPLY_NO=CC.APPLY_NO
WHERE CC.COMPANY='DSCG' AND CC.ISLINK=0 AND CC.ISLINK=0
ORDER BY CC.ID ASC,AA.APPLY_NO ASC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                headerText = string.Format("{0}暂借{1}{2} {3}", headerapply_DisplayName, headerpay_obj_name, headerabstract, findisplayname);
                //}
                //else
                //{
