...
--AA.SAP_VEN_CODE,
DBO.GETSAPFININFOCODE(CC.PAY_OBJ_ID,'DSCG',1) SUPP_ABB,
...
LEFT JOIN DABAN_BPM_DSCG.DBO.ZSK_C_H CC ON AA.ZSK_NO=CC.APPLY_NO
LEFT JOIN BPMDB.DBO.SAP_COMPANYFUNDS_LINKS_QUEUE DD ON AA.APPLY_NO=DD.APPLY_NO
LEFT JOIN (SELECT TASKID,SUM(APPLY_AMOUNT) APPLY_AMOUNT FROM DABAN_BPM_DSCG.DBO.FKTZS_ZSK_D GROUP BY TASKID) EE ON AA.TASKID=EE.TASKID
...
ORDER BY DD.ID ASC,AA.APPLY_NO ASC,BB.ID ASC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                headerText = string.Format("{0}销账{1}{2} {3}", headerapply_DisplayName,headerpay_obj_name, headerabstract, findisplayname);
