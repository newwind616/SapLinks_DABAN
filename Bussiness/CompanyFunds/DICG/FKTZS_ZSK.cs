...
--AA.SAP_VEN_CODE,
DBO.GETSAPFININFOCODE(CC.PAY_OBJ_ID,'DICG',1) SUPP_ABB,
CC.DK_PAY_OBJ_NAME,CC.PAY_MODE,
dbo.getFINDisplayName(AA.TASKID) FINDISPLAYNAME,
AA.ABSTRACT
FROM DABAN_BPM_DICG.DBO.FKTZS_C_H AA
LEFT JOIN DABAN_BPM_DICG.DBO.FKTZS_KM_C_D BB ON AA.TASKID=BB.TASKID
LEFT JOIN DABAN_BPM_DICG.DBO.ZSK_C_H CC ON AA.ZSK_NO=CC.APPLY_NO
LEFT JOIN BPMDB.DBO.SAP_COMPANYFUNDS_LINKS_QUEUE DD ON AA.APPLY_NO=DD.APPLY_NO
LEFT JOIN (SELECT TASKID,SUM(APPLY_AMOUNT) APPLY_AMOUNT FROM DABAN_BPM_DICG.DBO.FKTZS_ZSK_D GROUP BY TASKID) EE ON AA.TASKID=EE.TASKID
...
ORDER BY DD.ID ASC,AA.APPLY_NO ASC,BB.ID ASC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                headerText = string.Format("{0}报销{1} {2}", apply_DisplayName, _abstractABB, findisplayname);
                bodyText = string.Format("{0}报销{1} {2}", apply_DisplayName, _abstract, findisplayname);
