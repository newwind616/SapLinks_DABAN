...
namespace SAPLinks.Bussiness.CompanyFunds.DICW
{
    ...
    {
        public void GetData(DICW_Action dics_Action)
        {
...
AA.SAP_CUST_CODE,
DBO.GETSAPFININFOCODE(AA.SAP_CUST_CODE,'DICW',2) CSR_ABB,
--DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWBS,'',0) SAP_VEN_NEWBS,
--DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWUM,'',0) SAP_VEN_NEWUM,
--AA.SAP_VEN_CODE,
DBO.GETSAPFININFOCODE(CC.PAY_OBJ_ID,'DICW',1) SUPP_ABB,
CC.DK_PAY_OBJ_NAME,CC.PAY_MODE,
dbo.getFINDisplayName(AA.TASKID) FINDISPLAYNAME
FROM DABAN_BPM_DICW.DBO.FKTZS_C_H AA
LEFT JOIN DABAN_BPM_DICW.DBO.FKTZS_KM_C_D BB ON AA.TASKID=BB.TASKID
LEFT JOIN DABAN_BPM_DICW.DBO.ZSK_C_H CC ON AA.ZSK_NO=CC.APPLY_NO
LEFT JOIN BPMDB.DBO.SAP_COMPANYFUNDS_LINKS_QUEUE DD ON AA.APPLY_NO=DD.APPLY_NO
LEFT JOIN (SELECT TASKID,SUM(APPLY_AMOUNT) APPLY_AMOUNT FROM DABAN_BPM_DICW.DBO.FKTZS_ZSK_D GROUP BY TASKID) EE ON AA.TASKID=EE.TASKID
...
DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWUM,'',0) SAP_VEN_NEWUM,
AA.SAP_VEN_CODE,
DBO.GETSAPFININFOCODE(AA.SAP_VEN_CODE,'DICW',1) SUPP_ABB 
FROM DABAN_BPM_DICW.DBO.FKTZS_C_H AA
