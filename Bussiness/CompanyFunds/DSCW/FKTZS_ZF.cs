...
ORDER BY CC.ID ASC,AA.APPLY_NO ASC,BB.ID ASC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                //if (pay_obj_type == "0")
                //{
                //headerText = bodyText = string.Format("{0}支付{1}：{2}", apply_DisplayName, pay_obj_name, _abstract);
                headerText = string.Format("{0}支付{1}：{2} {3}初审", apply_DisplayName.SubstringExt(4), pay_obj_name.SubstringExt(8), _abstract.SubstringExt(4), findisplayname.SubstringExt(3));
