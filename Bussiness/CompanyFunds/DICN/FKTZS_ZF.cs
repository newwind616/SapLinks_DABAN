            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (pay_obj_type == "0")
                {
                    ...
                    if (pay_mode == "打卡")
                    {
                        ...
                        bodyText = string.Format("{0}{1}申请{2}{3}费用", apply_DisplayName, apply_Date.ToString("yyyy年MM月").Substring(2), dk_pay_obj_name, sap_ven_code + sap_cust_code);
                    }
                    else
                    {
                        headerText = string.Format("{0}{1}申请{2}费用 {3}", apply_DisplayName, apply_Date.ToString("yyyy年MM月").Substring(2), supp_abb + csr_abb, findisplayname);
                ...
                ConvertAccount(dicn_Action.company,sap_ven_code, account_No, loan_Sign, out NEWKO, out NEWBS, out NEWUM);
