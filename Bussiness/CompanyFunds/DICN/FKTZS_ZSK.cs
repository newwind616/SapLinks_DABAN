...
namespace SAPLinks.Bussiness.CompanyFunds.DICN
{
    ...
    {
        public void GetData(DICN_Action dicn_Action)
        {
...
AA.SAP_CUST_CODE,
DBO.GETSAPFININFOCODE(AA.SAP_CUST_CODE,'DICN',2) CSR_ABB,
--DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWBS,'',0) SAP_VEN_NEWBS,
--DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWUM,'',0) SAP_VEN_NEWUM,
--AA.SAP_VEN_CODE,
DBO.GETSAPFININFOCODE(CC.PAY_OBJ_ID,'DICN',1) SUPP_ABB,
CC.DK_PAY_OBJ_NAME,CC.PAY_MODE,
dbo.getFINDisplayName(AA.TASKID) FINDISPLAYNAME 
FROM DABAN_BPM_DICN.DBO.FKTZS_C_H AA
LEFT JOIN DABAN_BPM_DICN.DBO.FKTZS_KM_C_D BB ON AA.TASKID=BB.TASKID
LEFT JOIN DABAN_BPM_DICN.DBO.ZSK_C_H CC ON AA.ZSK_NO=CC.APPLY_NO
LEFT JOIN BPMDB.DBO.SAP_COMPANYFUNDS_LINKS_QUEUE DD ON AA.APPLY_NO=DD.APPLY_NO
LEFT JOIN (SELECT TASKID,SUM(APPLY_AMOUNT) APPLY_AMOUNT FROM DABAN_BPM_DICN.DBO.FKTZS_ZSK_D GROUP BY TASKID) EE ON AA.TASKID=EE.TASKID
...
ORDER BY DD.ID ASC,AA.APPLY_NO ASC,BB.ID ASC";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                ConvertAccount(dicn_Action.company,sap_cust_code, account_No, loan_Sign, out NEWKO, out NEWBS, out NEWUM);
                ...
                if ((i != dt.Rows.Count - 1 && dt.Rows[i]["APPLY_NO"].ToString() != dt.Rows[i + 1]["APPLY_NO"].ToString()) || (i == dt.Rows.Count - 1))
                {
                    ...
                    list.Add(currency);//货币码（WAERS）									
                    list.Add("");//汇率（KURSF）
                    AddZFLB(apply_No,bodyText,list,dicn_Action.company);
                }
            }
        }
...
DBO.GETSAPFININFOCODE(AA.SAP_VEN_NEWUM,'',0) SAP_VEN_NEWUM,
AA.SAP_VEN_CODE,
DBO.GETSAPFININFOCODE(AA.SAP_VEN_CODE,'DICN',1) SUPP_ABB 
FROM DABAN_BPM_DICN.DBO.FKTZS_C_H AA
