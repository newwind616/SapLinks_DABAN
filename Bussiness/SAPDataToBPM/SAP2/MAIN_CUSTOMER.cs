...
namespace SAPLinks.Bussiness.SAPDataToBPM.SAP2
{
    public class MAIN_CUSTOMER : SAPDataToBPMObject
    {
        ...
        {
            LogInfo.Log.Info("执行SAP2客户行主数据同步");
            StringBuilder errMsg = new StringBuilder();
            StringBuilder successMsg = new StringBuilder();
            SQLHelper.ExecuteNonQuery(context.connStr, "DELETE FROM MAIN_CUSTOMER WHERE SUPP_TYPE=0 AND COMPANY IN(" + filter + ")");
            DataTable mainsupplier = SQLHelper.ExecuteDataset(context.connStr, CommandType.Text, "SELECT COMPANY,SUPP_ID,SUPP_NAME,SUPP_ABB,SUPP_TYPE,BANK_NAME,BANK_SUB_NAME,BANK_ACCOUNT,DEL_FLG,SUPP_CODE FROM MAIN_CUSTOMER WHERE COMPANY IN(" + filter + ")").Tables[0];
            //向表中添加数据
            DataRow dr;
