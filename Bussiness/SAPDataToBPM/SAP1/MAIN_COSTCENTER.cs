...
namespace SAPLinks.Bussiness.SAPDataToBPM.SAP1
{
    public class MAIN_COSTCENTER : SAPDataToBPMObject
    {
        ...
        {
            LogInfo.Log.Info("执行SAP1成本中心主数据同步");
            ...
            using (StreamReader sr = new StreamReader(filePath, Encoding.UTF8))
            {
                ...
                for (int i = 0; i < strlist.Length; i++)
                {
                    ...
                    if (strs[0] == "2060")//特殊公司取编码逻辑调整：10个0的取0，其他取最后3位
                    {
                        string longCode = strs[1];
                        if (longCode.Length < 3)
                            throw new Exception(string.Format("{0}公司检测到有3位以为下的编码:{1}", dr["COMPANY"], longCode));
                        else if (longCode == "0000000000")
                            dr["CODE"] = "0";
                        else
                            dr["CODE"] = longCode.Substring(longCode.Length-3,3);
                    }
                    else
                    {
                        dr["CODE"] = strs[1].TrimStart('0');
                    }
                    dr["NAME"] = strs[2];
