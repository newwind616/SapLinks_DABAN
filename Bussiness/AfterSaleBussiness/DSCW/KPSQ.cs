...
namespace SAPLinks.Bussiness.AfterSaleBussiness.DSCW
{
    public class KPSQ : AfterSaleObject
    {
        ...
        public override void GetData()
        {
            ...
            LogInfo.Log.Info(string.Format("开票申请销售数量:{0}", this.KPSQSaleCount));
            LogInfo.Log.Info(string.Format("开票申请采购数量:{0}", this.KPSQPurchaseCount));
            LogInfo.Log.Info(string.Format("开票申请冲销销售数量:{0}", this.KPSQCXSaleCount));
            LogInfo.Log.Info(string.Format("开票申请冲销采购数量:{0}", this.KPSQCXPurchaseCount));
            LogInfo.Log.Info(string.Format("开票申请非DCOS销售数量:{0}", this.KPSQNotDcosSaleCount));
            LogInfo.Log.Info(string.Format("开票申请非DCOS采购数量:{0}", this.KPSQNotDcosPurchaseCount));
        }
	...
	select AA.TASKID,AA.FinishAt,AA.SerialNum,AA.XH,BB.Z_XH,BB.Z_ZP,BB.Z_GLBH,BB.Z_GLBH_PK,BB.Z_HTBH,AA.KHBM Z_KHBM,AA.KHMC Z_KHMC,BB.Z_YWLX,BB.Z_YYDD,BB.S_MC,BB.S_DJ,BB.S_KPJE,BB.S_SL,BB.S_BHSJE,BB.C_SAKNR,BB.C_SNWBMC,BB.C_WBJE,BB.C_WBCS,BB.C_CXZBBB,BB.CD,BB.MAIL,BB.MOBILE from (
		...
		select AA.TASKID,AA.FinishAt,AA.SerialNum,BB.ID,BB.XH,BB.KHBM,BB.KHMC from (
			select AA.TASKID,BB.FinishAt,BB.SerialNum from DABAN_BPM_DSCW.[dbo].[KPSQ_C_H] AA 
		...
		) AA left join DABAN_BPM_DSCW.[dbo].[KPSQ_FP_D] BB on AA.TASKID=BB.TASKID
		--开票信息，开票序号 结束
	) AA left join DABAN_BPM_DSCW.[dbo].[KPSQ_ZYDXX_D] BB on AA.TASKID=BB.TASKID and AA.ID=BB.KPSQ_FP_D_ID
	--left join BPMDB.dbo.MAIN_CUSTOMER CC on AA.KHBM = CC.CSR_ID and CC.COMPANY='DSCW'
...
where BB.ISLINK=0";
        ...
        #region 非DCOS生成采购、销售数据
        private void DoNotDCOS()
        {
            /************************查询中添加申请人工号字段——AA.APPLY_ACCOUNT*****************************/
            /************************查询中添加KPSQ_ZYDXX_D的字段：Z_KHMC*****************************/
            /************************查询中添加KPSQ_FP_D的字段：KHDCHM*****************************/
            string sql = @"select AA.* from (
	select AA.TASKID,AA.FinishAt,AA.SerialNum,AA.APPLY_ACCOUNT,AA.XH,AA.KHDCHM,BB.Z_XH,BB.Z_ZP,BB.Z_GLBH,BB.Z_GLBH_PK,BB.Z_HTBH,AA.KHBM Z_KHBM,AA.KHMC Z_KHMC,BB.Z_KHMC ZYDXX_KHMC,BB.Z_YWLX,BB.Z_YYDD,BB.S_MC,BB.S_DJ,BB.S_KPJE,BB.S_SL,BB.S_BHSJE,BB.C_SAKNR,BB.C_SNWBMC,BB.C_WBJE,isnull(BB.C_WBCS,0) C_WBCS,BB.C_CXZBBB,BB.CD,BB.MAIL,BB.MOBILE from (
		--开票信息，开票序号 开始
		select AA.TASKID,AA.FinishAt,AA.SerialNum,AA.APPLY_ACCOUNT,BB.ID,BB.XH,BB.KHBM,BB.KHMC,isnull(BB.KHDCHM,'') KHDCHM from (
			select AA.TASKID,BB.FinishAt,BB.SerialNum,AA.APPLY_ACCOUNT from DABAN_BPM_DSCW.[dbo].[KPSQ_C_H] AA 
			left join BPMDB.DBO.BPMInstTasks BB on AA.TASKID=BB.TaskID
			where AA.YCWC_TYPE='3' and BB.State='Approved'
		) AA left join DABAN_BPM_DSCW.[dbo].[KPSQ_FP_D] BB on AA.TASKID=BB.TASKID
		--开票信息，开票序号 结束
	) AA left join DABAN_BPM_DSCW.[dbo].[KPSQ_ZYDXX_D] BB on AA.TASKID=BB.TASKID and AA.ID=BB.KPSQ_FP_D_ID
	--left join BPMDB.dbo.MAIN_CUSTOMER CC on AA.KHBM = CC.CSR_ID and CC.COMPANY='DSCW'
...
where BB.ISLINK=0";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (Convert.ToDecimal(dt.Rows[i]["C_WBCS"]) > 0 && VerificationPurchaseSaknr(Convert.ToString(dt.Rows[i]["C_SAKNR"])))
                {
                    DoPurchase_NotDCOS(dt.Rows[i]);
                    KPSQNotDcosPurchaseCount++;
                }

                DoSale_NotDCOS(dt.Rows[i]);
                KPSQNotDcosSaleCount++;
            }
        }
        #region ＜SAP连携(采购-借 31)＞ ＜SAP连携(采购-贷 40)＞
        private void DoPurchase_NotDCOS(DataRow dr)
        {
            list_purchase.Clear();
            string z_glbh = Convert.ToString(dr["Z_GLBH"]);
            //string sql = @"select {3} from [DABAN_BPM_{0}].[dbo].[MAIN_PURCHASE_I]  where SAKNR like '{1}%'and CXZBBB='{2}'";
            string cd = Convert.ToString(dr["CD"]);
            string cxzbbb = Convert.ToString(dr["C_CXZBBB"]);
            //string kostlSQL = string.Format(sql, cd, "410101150", cxzbbb, "KOSTL");
            // string kostl = Convert.ToString(SQLHelperExt.ExecuteFirstValue(context.connStr, kostlSQL));//采购文件4101011502这个科目明细中的第2个字段
            ...
            string s_sws = string.Format("select  ISNULL(FD_SWS,'') SWS  from  BPMDB.DBO.BPMSYSUSERS where Account='{0}'", Convert.ToString(dr["APPLY_ACCOUNT"]));
            string sws = Convert.ToString(SQLHelperExt.ExecuteFirstValue(context.connStr, s_sws));//成本中心（KOSTL） 申请人对应的事务所代码	【BPMSysUsers.SWS】中取得（申请者对应数据）
            # region 采购-借 31
            list_purchase.Add(z_glbh);//参照号（XBLNR）
            list_purchase.Add(finishat);//凭证日期（BLDAT）
            list_purchase.Add(finishat);//记账日期（BUDAT）
            ...
            list_purchase.Add(z_glbh);//分配（ZUONR）
            list_purchase.Add(sgtxt);//明細テキスト（SGTXT）
            list_purchase.Add("");//取引先参照キー１（XREF1）
            list_purchase.Add("");//取引先参照キー２（XREF2）
            list_purchase.Add("");//取引先参照キー３（XREF3）
            list_purchase.Add(finishat);//支払基準日（ZFBDT）
            list_purchase.Add("");//支払条件（ZTERM）
            ...
            list_purchase.Add("");//特殊总账标识（NEWUM）
            list_purchase.Add("");//資産取引タイプ（NEWBW）
            list_purchase.Add(sws);//成本中心（KOSTL） 申请人对应的事务所代码	【BPMSysUsers.SWS】中取得（申请者对应数据）																	
            list_purchase.Add(sws);//利润中心（PRCTR） 申请人对应的事务所代码	【BPMSysUsers.SWS】中取得（申请者对应数据）				
            list_purchase.Add("");//WBS要素（PROJK）
            list_purchase.Add("");//内部订单号（AUFNR）
            list_purchase.Add(c_wbcs.ToString());//凭证货币金额（WRBTR）
            list_purchase.Add("");//本地货币金额（DMBTR）
        ...
        #endregion ＜SAP连携(采购-借 31)＞ ＜SAP连携(采购-贷 40)＞

        private void DoSale_NotDCOS(DataRow dr)
        {
            list_sale.Clear();
            #region 查询非DOCS数据获得
            string cd = Convert.ToString(dr["CD"]);
            ...
            //DataTable dt = SQLHelper.ExecuteDataset(context.connStr, System.Data.CommandType.Text, sql).Tables[0];
            //sql = string.Format("select CITY2 from DABAN_BPM_{0}.dbo.MAIN_INCOME_H where CBXZ='{1}'", cd, z_glbh_pk);//客户国家代码
            string kna1land1 = "CN";//Convert.ToString(SQLHelperExt.ExecuteFirstValue(context.connStr, sql));
            //string bstzd2 = Convert.ToString(dt.Rows[0]["BSTZD2"]);//原价区分
            //string vtweg = Convert.ToString(dt.Rows[0]["VTWEG"]);//分销渠道
            //string city1 = Convert.ToString(dt.Rows[0]["CITY1"]);//客户城市
            //string waerk = Convert.ToString(dt.Rows[0]["WAERK"]);//凭证货币
            ...
            string mobile = Convert.ToString(dr["MOBILE"]);
            #endregion
            string z_glbh = Convert.ToString(dr["Z_GLBH"]); //开票申请的作业单号
            string z_khbm = Convert.ToString(dr["Z_KHBM"]);
            string z_khmc = Convert.ToString(dr["Z_KHMC"]);
            string zydxx_khmc = Convert.ToString(dr["ZYDXX_KHMC"]);
            //string kna1land1 = Convert.ToString(dr["KNA1LAND1"]);//客户国家代码
            string serialnum = Convert.ToString(dr["SerialNum"]);//
            string xh = Convert.ToString(dr["XH"]);
            string zxh = Convert.ToString(dr["Z_XH"]);
            ...
            decimal s_kpje = Convert.ToDecimal(dr["S_KPJE"]);
            string z_ywlx = Convert.ToString(dr["Z_YWLX"]);
            string khdchm = string.IsNullOrEmpty(Convert.ToString(dr["KHDCHM"])) ? "-" : Convert.ToString(dr["KHDCHM"]);//客户城市
            ...
            list_sale.Add("0");//正负
            list_sale.Add("D");//DRBR区分
            list_sale.Add(z_ywlx);//原价区分
            list_sale.Add(sap_cd);//销售组织 固定值"DSC=4000，DSCW=4020，DSCB=4010,DSCG=4030，DSCW-=4040"
            list_sale.Add("D0");//分销渠道
            list_sale.Add(z_khbm);//客户编码
            list_sale.Add(z_khmc);//客户名称
            list_sale.Add(khdchm);//客户城市
            list_sale.Add(kna1land1);//客户国家代码
            list_sale.Add(serialnum);//客户的采购号 开票申请编号+发票序号
            list_sale.Add("CNY");//凭证货币
            list_sale.Add(finishat);//定价日期 开票申请终审日期
            list_sale.Add("");//机种
            list_sale.Add(z_glbh);//销售单号 开票申请编号+发票序号
            list_sale.Add(sws);//联络所 申请人对应的事务所代码
            list_sale.Add(sws);//利润中心 申请人对应的事务所代码
            list_sale.Add(z_ywlx);//事业别
            list_sale.Add(z_yydd);//营业担当 开票申请中的“营业担当”
            list_sale.Add("");//客户电话号码 
            if (mailMobile)
            {
                list_sale.Add(mail);//邮箱
                list_sale.Add(mobile);//手机号 
            ...
            list_sale.Add(z_glbh);//管理 / 销售编号
            list_sale.Add("10");//明细号
            string wlh = "ZDSC_SERV_1";
            if (zydxx_khmc.Trim() == "设备")
            {
                wlh = "ZDSC_FERT_1";
            }
            else if (zydxx_khmc.Trim() == "工作服")
            {
                wlh = "ZDSC_PROT_1";
            }
            else
            ...
            list_sale.Add(wlh);//物料号 KPSQ_ZYDXX_D.Z_KHMC='安装'：ZDSC_SERV_1； KPSQ_ZYDXX_D.Z_KHMC = '设备'：ZDSC_FERT_1
            list_sale.Add(s_mc);//物料描述
            list_sale.Add(sap_cd);//工厂 固定值"DSC =4000，DSCW=4020，DSCB=4010,DSCG=4030，DSCW-=4040"
            list_sale.Add("1");//数量
            list_sale.Add("PC");//数量单位
            list_sale.Add(s_kpje.ToString());//含税总价
            list_sale.Add(s_sl.ToString());//税率百分数
