...
namespace SAPLinks.Bussiness.AfterSaleBussiness.WC
{
    ...
    /// 生成成本-基本生产成本-外注费-有偿
    /// </summary>
    public class CostAdjustmentAC : ACostAdjustmentAC
    {
        protected override List<AccVouch> DoLoad()
        {
            ...
            accVouch.NEWKO = context.Fktzs_C_HEntitys.PayObjId;//客户 / 供应商 / 会计科目代码（NEWKO）//付款通知书选择支付对象的供应商代码
            accVouch.NEWBS = "39";//记账码（NEWBS）
            accVouch.NEWUM = "L";//特殊总账标识（NEWUM）
            ...
            accVouch.WRBTR = context.Fktzs_C_HEntitys.SjjeTotal.ToString().Abs();//凭证货币金额（WRBTR）//表单中的实际支付金额合计
