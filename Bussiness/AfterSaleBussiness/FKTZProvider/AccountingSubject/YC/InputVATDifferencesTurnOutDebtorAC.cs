...
namespace SAPLinks.Bussiness.AfterSaleBussiness.YC
{
    ...
    /// 应交税费-增-进项转出
    /// </summary>
    public class InputVATDifferencesTurnOutDebtorAC : AInputVATDifferencesTurnOutDebtorAC
    {
        protected override List<AccVouch> DoLoad()
        {
            ...
            accVouch.NEWKO = "4101011502";//客户 / 供应商 / 会计科目代码（NEWKO）//付款通知书选择支付对象的供应商代码
            accVouch.NEWBS = GetNEWBS();//记账码（NEWBS）
            accVouch.NEWUM = "";//特殊总账标识（NEWUM）
            accVouch.NEWBW = "";//資産取引タイプ（NEWBW）
            accVouch.KOSTL = context.Fktzs_C_HEntitys.ApplyDeptCode;//成本中心（KOSTL）
